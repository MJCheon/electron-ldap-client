<template>
  <div class="vtl">
    <div
      v-if="model.name !== 'root' && model.isVisible"
      :id="model.id"
      class="vtl-node"
    ...
    <div
      ...
      v-show="model.name === 'root' || model.isExpanded"
      v-if='isFolder'
    >
      <item
...
export default {
  ...
  data: function () {
    return {
      ...
      expanded: this.defaultExpanded || this.model.isExpanded
    }
  },
  props: {
    model: {
  ...
  watch: {
    search: {
      handler () {
        traverseFilteredTree(this.model, this.search)
      },
      deep: true
    }
  },
